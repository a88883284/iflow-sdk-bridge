...
**问题描述**: iFlow CLI SDK 有自己的系统提示词处理机制，可能与你的 AI 工具的提示词产生冲突。

| 问题 | 说明 |
|------|------|
| **iFlow 系统提示词注入** | SDK 会自动附加 iFlow 默认的系统提示词 |
| **客户端提示词被过滤** | SDK 可能过滤或覆盖来自 OpenClaw/Claude Code 的提示词 |
| **工具调用异常** | 自定义工具定义可能被忽略或导致错误 |
| **对话偏离** | AI 行为可能与预期的人设不符 |

**根本原因**: `@iflow-ai/iflow-cli-sdk` 是为 iFlow CLI 生态系统设计的，不是通用的 API 代理。它会强制执行自己的会话设置。

**当前解决方案**:
- 本桥接器最适合**简单对话场景**，不需要复杂系统提示词
- 对于需要自定义工具/提示词的 AI 代理，建议使用 [iflow2api](https://github.com/cacaview/iflow2api) 替代

**状态**: 正在调查中。欢迎 PR 来改进提示词处理。
